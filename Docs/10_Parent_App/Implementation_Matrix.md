...
| P-HEALTH-001 | Devices | Device health (heartbeat/last seen) | Win✅ | 🟡 |  26W08: consolidated 'Health' tab in child profile (policy sync + status + diagnostics summary) built on /status + /devices + diagnostics info. Per-device deep health + diagnostics packaging pipeline still pending. |
