...
| EPIC-PAIRING | Kid/Devices | Pairing & provisioning (QR/code/deeplink) | 🟡 | Parent: pairing UX + device registry; Kid: pairing endpoint stub | SSOT: device registry + enrollment tokens; **token revoke + expiry** | Implemented in 16W10–16W14 (minimal); hardened in 16W20 (token revoke/expiry) |
| EPIC-POLICY-SYNC-RUNTIME | Kid Agent | Versioned sync + watchdog + rollback/integrity | 🟡 | 16W19: policyVersion bump on save + agent ack persisted + UI surfaces applied version. 16W21: watchdog surfaces Pending/Overdue state + pendingSince. Rollback/integrity still planned. | SSOT: policy versions + heartbeat | KFT-006 (partial) |
