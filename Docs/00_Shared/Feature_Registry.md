...
| EPIC-POLICY-SYNC-RUNTIME | Kid Agent | Versioned sync + watchdog + rollback/integrity | 🟡 | Kid agent + Parent UI (Devices tab) | Watchdog + apply error observability shipped; **26W09** adds policy-sync health counters + best-effort enroll-by-code self-repair when token is rejected or policy fetch fails persistently. Full integrity/self-repair remains planned. | Evidence: `src/Safe0ne.ChildAgent/Policy/PolicySyncHealthStore.cs`, `src/Safe0ne.ChildAgent/HeartbeatWorker.cs` (TamperSignals fields), Parent UI: `src/Safe0ne.DashboardServer/wwwroot/app/features/children.js` (“Policy sync health” card). |
...
| EPIC-AUDIT | Parent | Audit log viewer (append-only policy changes) | 🟢 | Admin→Audit Log loads from SSOT; policy PUT/PATCH and device token revoke append entries | SSOT: append-only audit | 26W09: viewer polish (filters/search/export) + retention purge endpoint |
