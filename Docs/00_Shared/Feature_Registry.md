...
| EPIC-POLICY-SYNC-RUNTIME | Kid Agent | Versioned sync + watchdog + rollback/integrity | 🟡 | Kid agent | 26W08: cached policy fallback + replay protection + watchdog surfacing + **self-repair backoff + health counters**; remaining: automated re-enroll + deeper integrity attestation | KFT-006+ |
...
| EPIC-ACTIVITY | Telemetry | Activity capture + retention + export | ✅ | Kid agent + Parent reports | SSOT: activity logs + retention policy | 26W08: ActivityOutbox (kid) + ControlPlane activity store + export envelope + Reports digests |
