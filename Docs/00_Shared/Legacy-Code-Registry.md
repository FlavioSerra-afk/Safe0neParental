...
| DashboardServer HTTP API | `Program.cs` – `GET /api/v1/childre_{id}/status` | Malformed route observed from cached/older UI assets (`children/` segment missing and leading underscore in id). Shim prevents repeated 404 noise in console by returning 200 with an application-level error. | Canonical `GET /api/v1/children/{id}/status` | All clients confirmed to call canonical status route; remove compat. |
