...
### CP01-B (completed)
- Extracted **Children**, **Requests**, and **Local Settings/Meta** internal methods into their domain partials.
- Kept all Program/endpoint-facing signatures unchanged.

### CP01-C (completed)
- Extracted `ReportsDigest` into a dedicated module: `src/Safe0ne.DashboardServer/Reports/ReportsDigest.cs`.
- `ReportSchedulerService` remains a thin hosted-service wrapper.

### CP01-D (completed)
- Extracted **Local Mode Activity** members into `JsonFileControlPlane.Activity.cs` (append/query + retention filtering).
- No API/signature changes; pure extraction.
...
- `src/Safe0ne.DashboardServer/ControlPlane/JsonFileControlPlane.Requests.cs`
- `src/Safe0ne.DashboardServer/ControlPlane/JsonFileControlPlane.Alerts.cs`
- `src/Safe0ne.DashboardServer/ControlPlane/JsonFileControlPlane.Activity.cs`
- `src/Safe0ne.DashboardServer/Reports/ReportsDigest.cs`

**Rule reminder**
- Extract helpers/internal logic first.
- Keep endpoint-facing methods/signatures stable until migration is explicitly planned and tested.
