...
public sealed class HeartbeatWorker : BackgroundService
{
    ...
    // 26W09L: avoid rollback loops (best-effort, process-lifetime)
    private int? _lastRolledBackPolicyVersion;
...
if (auth?.DeviceToken is not null)
                ...
                if (policyVersion is not null && lastAppliedPolicyVersion is not null && policyVersion.Value < lastAppliedPolicyVersion.Value)
                {
	                ...
	                var cacheLoad = PolicyCacheStore.Load(childId);
	                var cached = cacheLoad.Entry;
	                bool cacheIntegrityOk = cacheLoad.IntegrityOk;
	                bool cacheCorrupt = !cacheIntegrityOk;
	                DateTimeOffset? cacheCorruptAtUtc = null;

	                if (cacheCorrupt)
	                {
	                    cacheCorruptAtUtc = DateTimeOffset.UtcNow;
	                    // Best-effort recovery: delete corrupt cache and attempt restore from .bak.
	                    PolicyCacheStore.Delete(childId);
	                    if (PolicyCacheStore.RestoreBackup(childId))
	                        cached = PolicyCacheStore.Load(childId).Entry;
	                    ...
	                    try
	                    {
	                        ...
	                        activityOutbox.Enqueue(new ActivityOutbox.LocalActivityEvent(
	                            ...
	                            {
	                                note = "Policy cache failed integrity check; cache cleared and backup restore attempted"
...
// K6/K8E: web filtering applies hosts rules and emits privacy-first aggregates.
                ...
                // 26W09L: cache integrity + rollback observability
                bool rollbackApplied = false;
                DateTimeOffset? rollbackAppliedAtUtc = null;
                string? rollbackReason = null;

                // Best-effort agent-side rollback: if we observe an enforcement error shortly after applying
                // a new policy version, restore the previous cached policy from backup.
                if (_lastEnforcementErrorAtUtc is not null
                    && lastAppliedPolicyVersion is not null
                    && (nowUtc - _lastEnforcementErrorAtUtc.Value) < TimeSpan.FromMinutes(5)
                    && (_lastRolledBackPolicyVersion is null || _lastRolledBackPolicyVersion.Value != lastAppliedPolicyVersion.Value))
                {
                    if (PolicyCacheStore.RestoreBackup(childId))
                    {
                        rollbackApplied = true;
                        rollbackAppliedAtUtc = DateTimeOffset.UtcNow;
                        rollbackReason = "enforcement_error_restore_backup";
                        _lastRolledBackPolicyVersion = lastAppliedPolicyVersion.Value;
                        ...
                        try
                        {
                            ...
                            activityOutbox.Enqueue(new ActivityOutbox.LocalActivityEvent(
                                EventId: Guid.NewGuid(),
                                OccurredAtUtc: DateTimeOffset.UtcNow,
                                Kind: "policy_rollback_applied",
                                App: null,
                                Details: JsonSerializer.Serialize(new
                                {
                                    policyVersion = lastAppliedPolicyVersion,
                                    note = "Restored previous cached policy backup after enforcement error"
                                }, JsonDefaults.Options),
                                DeviceId: auth?.DeviceId.ToString()));
                        }
                        catch { }
                ...
                var tamper = new TamperSignals(
                    ...
                    PolicyCacheCorrupt: cacheCorrupt,
                    PolicyCacheCorruptAtUtc: cacheCorruptAtUtc,
                    PolicyRollbackApplied: rollbackApplied,
                    PolicyRollbackAtUtc: rollbackAppliedAtUtc,
                    PolicyRollbackReason: rollbackReason);
